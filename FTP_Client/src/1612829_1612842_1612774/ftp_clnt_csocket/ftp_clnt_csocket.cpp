...
private:
	...
	char* getPort()
	{
		...
		//sprintf(port, "PORT %s.%d.%d", this->ip, port1, port2);
...
private:
	void receiveData(CSocket &dataTrans)
	{
		...
		while (len == BUFSIZ) {
			if (FD_READ) {
				msg = new char[BUFSIZ + 1];
				len = dataTrans.Receive((char*)msg, len, 0);
				sscanf(msg, "%d", &error);
				msg[len] = '\0';
				printf("%s", msg);
				delete[] msg;
	...
	void receiveDataLoadFile(CSocket &dataTrans, FILE* f)
	{
		...
		while (len == BUFSIZ)
		{
			if (FD_READ) {
				msg = new char[BUFSIZ + 1];
				len = dataTrans.Receive(msg, len, 0);
				fwrite(msg, len, 1, f);
				delete[] msg;
			}
			else {
	...
	void Process() {
		...
		while (1) {
			...
			if (this->isConnected) {
				...
				if ((double)(finish-start)/CLOCKS_PER_SEC > 15) {
